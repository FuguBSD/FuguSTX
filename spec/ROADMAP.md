...
| P11   | The pair corpus: the sources at pinned tags, the memorization check, the two generators, the judge filter, and the zero-training baseline of decision T13.               |
| P12   | The first SFT campaign on the pair corpus. The baseline scorecard fixes the tier T1 thresholds, and the T13 bar decides the pilot.                                       |
| P13   | `stx analyze` against one FuguBSD repository beside the regex `ste-lint`, and the later-era human set measurement.                                                       |

P11 precedes every later training phase. P7 and P8 run on the pair corpus after
P12. P5 is independent, and a phase of it can interleave with the others.
